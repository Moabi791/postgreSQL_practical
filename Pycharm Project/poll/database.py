...
Vote = Tuple[str, int]
PollWithOption = Tuple[int, str, int, float]

CREATE_POLLS = "CREATE TABLE IF NOT EXISTS polls (id SERIAL PRIMARY KEY, title TEXT, owner_userame TEXT);"
...
SELECT
    ...
    COUNT(votes.option_id) / SUM(COUNT(votes.options_id)) OVER () * 100.0 as vote_percentage
FROM options 
LEFT JOIN votes on options.id = votes.option_id
WHERE options.poll_id = %s
GROUP BY options.id;"""
SELECT_RANDOM_VOTE = "SELECT * FROM votes (username, option_id) VALUES (%s, %s)"
...
def get_latest_poll(connection, SELECT_LATEST_POLL_WITH_OPTIONS=None) -> List[PollWithOption]:
    with connection:
        with connection.cursor() as cursor:
...
def create_poll(connection, title: str, owner: str, options: List[str], INSERT_POLL_RETURN_ID=None):
    with connection:
        with connection.cursor() as cursor:
...
def add_poll_vote(connection, username: str, option_id: int, INSERT_VOTE=None):
    with connection:
        with connection.cursor() as cursor:
